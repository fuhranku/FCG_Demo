...
vec3 calcDirLight(DirLight light,vec3 normal, vec3 viewDir, vec2 texCoords, vec3 LightDirection);
...
void main(){
	...
	float ratio = material.ambient_reflective_index / material.material_reflective_index;
	vec3 I = normalize(fragPos - viewPos);
//    vec3 R = refract(I, normalize(norm), ratio);
	vec3 R = reflect(I, normalize(norm));

    //Final fragment
    FragColor =	vec4(result,1.0) + vec4(texture(skybox, R).rgb, 1.0f)*reflection;
}
...
vec3 calcDirLight(DirLight light,vec3 normal, vec3 viewDir, vec2 texCoords, vec3 LightDirection){
	...
	// Result	
	return (ambient + diffuse + specular) * texture(diffMap, texCoords).rgb;
}
